                                ...
                                int inputWidth, int inputHeight, int numFilters, int filterSize, int numImages)
{
  /**
   * * Add these
   * * input offset = inputwidth * inputheight * blockidx.x
   * * output offset = outputwidth * outputheight * numfilters * blockidx.x
   *
   * * for many images:
   * * for image=0,image<imagePerBlock;im++
   * * image offset = image * input offset
   *
   * * boundaries
   * * outputIndex < width * height * numFilters * totalNImages
   */

  {
    ...
    int imageIndex = blockIdx.x; // Each block handles one image
    if (imageIndex <= numImages && filterIndex < numFilters)
    {
      int inputOffsetX = imageIndex * inputWidth;
      int outputOffsetX = (imageIndex * numFilters + filterIndex) * outputWidth;
      ...
      float result = 0.0f;
      int outputIndex = (filterIndex * outputHeight + outputY) * outputWidth + outputX + (blockIdx.x * outputWidth * outputHeight * numFilters);
      for (int i = 0; i < filterSize; ++i)
      {
        for (int j = 0; j < filterSize; ++j)
        {
          ...
          int inputIndex = currentX * inputWidth + currentY + inputOffsetX;
          int filterIdx = (filterIndex * filterSize * filterSize) + (i * filterSize) + j;
          printf("(%d) result: %f + filterIdx:%d  %f x %f\n", outputIndex, result, filterIdx, input[inputIndex], filters[filterIdx]);
      ...
      output[outputIndex + outputOffsetX] = result;
      // }
    }
  }
}
...
int main()
{
  ...
  int numFiltersConv1 = 3; // 3 filters

  int imagesToShow = 3; // Change this value to display output for a different number of images
  ...
  // This should match the dimension of the output image
  dim3 blockSize(3, 3);
  dim3 numBlocks(imagesToShow, 1, numFiltersConv1);
  ...
  convolutionLayerMultipleFilters<<<numBlocks, blockSize>>>(d_input, d_output, d_filters,
                                                            ...
                                                            numFiltersConv1, filterSize, imagesToShow);
  ...
  for (int i = 0; i < imagesToShow * numFiltersConv1 * width * height; ++i)
  {
    ...
    if (i % (singleFilter) == 0)
      std::cout << "\n\nFilter " << (i / singleFilter) % numFiltersConv1;
    if (i % width == 0)
      std::cout << std::endl;
    std::cout << output[i] << " ";
  }
